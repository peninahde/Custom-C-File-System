...
int fs_format(const char *disk_path) {
    ...
    lseek(disk_fd, 1 * sb.block_size, SEEK_SET); //jump to the end of the superblock block

    write(disk_fd, bitmap, sizeof(bitmap)); //write the bitmap (block 1)
    lseek(disk_fd, 2 * sb.block_size, SEEK_SET); // jump to the end of the bitmap block
    write(disk_fd, inode_table, sizeof(inode_table)); //write the inode table (8 blocks)

    lseek(disk_fd, (MAX_BLOCKS * sb.block_size) - 1, SEEK_SET); //jump to the last byte of the 10MB file
...
void fs_unmount() {
    ...
    lseek(disk_fd, 1 * sb.block_size, SEEK_SET); //go to beginning of bitmap block
    write(disk_fd, bitmap, sizeof(bitmap)); //write the contents of bitmap to the disk

    // read inode table and write it to memory
    lseek(disk_fd, 2 * sb.block_size, SEEK_SET); //go to beginning of inode_table block
...
int fs_create(const char *filename) {
    ...
    sb.free_inodes--;
    write_back_sb();
    write_inode(free_inode_index, &inode_table[free_inode_index]);
    return 0;
}
...
int fs_read(const char *filename, void *buffer, int size) {
    ...
    while (total_bytes_read < bytes_to_read) {
        ...
        int block_we_are_in = total_bytes_read / sb.block_size; // (some number / 4096) should be a number 1 to 12
        int offset_within_block = total_bytes_read % sb.block_size; //once we are in the right block, go to the right place in it
        int bytes_left_in_current_block = sb.block_size - offset_within_block; //number of bytes we can read within the current block
        ...
        int size_to_read = (bytes_left_to_read < bytes_left_in_current_block) ? bytes_left_to_read : bytes_left_in_current_block;
        ...
        lseek(disk_fd, (physical_block * sb.block_size) + offset_within_block, SEEK_SET);

        //read the size_to_write into the users buffer
        int bytes_read = read(disk_fd, (char *)buffer + total_bytes_read, size_to_read);
...
void write_bitmap() {
    lseek(disk_fd, sb.block_size, SEEK_SET);
